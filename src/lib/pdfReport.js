...
/**
 * Generate an official single-expense Payment Bill PDF
 */
export async function generateSingleExpenseVoucherPDF({ project, expense, firmName, save = true }) {
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(...BLACK)
  doc.text('PAYMENT BILL', pageW - margin - pad, curY + 4, { align: 'right' })
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(...BLACK)
  doc.text('Project: ', margin + pad, curY)
  doc.setFont('helvetica', 'normal')
  doc.setTextColor(...DARK_GRAY)
  doc.text(`${project?.project_name || 'Project'} (${project?.project_code || '—'})`, margin + pad + 15, curY)
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(...DARK_GRAY)
  doc.text('Total Paid:', margin + pad + 3, curY + 10.5)
  doc.setFontSize(11)
  doc.setTextColor(...BLACK)
  ...
  doc.text('Generated by SiteLedger', pageW - margin, footerY + 5, { align: 'right' })

  const prefix = (firmName || 'SiteLedger').replace(/\s+/g, '_')
  const vName = (expense.vendor_name || 'Expense').replace(/\s+/g, '_')
  const filename = `${prefix}_Bill_${vName}_${new Date(expense.expense_date).toLocaleDateString('en-IN').replace(/\//g, '-')}.pdf`
